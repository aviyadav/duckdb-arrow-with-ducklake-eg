...
class Storage:
    """S3-compatible object storage, backed by RustFS."""
    ...
    def __init__(self, prefix: StoragePrefix):
        ...
        self.bucket_name = env.str("S3_BUCKET")
    ...
    def create(self):
        if self.exists():
            log.info("Bucket already exists: {}", self.bucket_name)
            return

        log.info("Creating bucket: {}", self.bucket_name)
        self.bucket.meta.client.create_bucket(Bucket=self.bucket_name)
