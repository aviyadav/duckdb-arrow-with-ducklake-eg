MODEL (
  name stage.dd.dataset,
  kind FULL,
  column_descriptions (
    example_id = 'Unique example identifier',
    input = 'Cleaned text of the social media post',
    target = 'Whether the post is about depression',
    is_test = 'Whether the example belongs to the test set'
  )
);
...
WITH dataset AS (
    ...
    FROM read_csv(
        @RAW__DEPRESSION_DETECTION__DEPRESSION_DATASET_REDDIT_TWITTER,
        delim = ',',
        quote = '"',
        escape = '"',
        header = true,
